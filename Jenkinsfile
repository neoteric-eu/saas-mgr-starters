...
node {
    ...
    if ('development'.equalsIgnoreCase(env.BRANCH_NAME)) {
        ...
        mvn "sonar:sonar -Dsonar.github.pullRequest=${env.CHANGE_ID} -Dsonar.github.repository=neoteric-eu/saas-mgr-starters -Dsonar.github.oauth=${env.SONAR_OAUTH} -Dsonar.analysis.mode=issues -B -e -V"
    } else {
